...
```{r}
library(Publish)
ci.mean(VarNum ~ VarQuali, Data = DATA)
```
...
## Classification Ascendante Hierarachique

<http://larmarange.github.io/analyse-R/classification-ascendante-hierarchique.html>

Package qui accelere la construction de la classification :
```{r}
library(fastcluster)
```

Package d'analyse géométrique de données qui permet aussi de construire des matrices de distances :
```{r}
library(ade4)
```
...
On commence par se représenter l'évolution de l'inertie selon le nombre de classes pour trouver d'eventuels paliers de façon visuelle :
```{r}
...
```
(Penser à ajuster le [1:9] selon le nombre de colonnes du tableau de données)
Sort le tableau des résidus du khi2 pour bien repérer les différences
...
Ce système de lettre peut ensuite être indexé et être réutilisé dans un graphique par exemple.
